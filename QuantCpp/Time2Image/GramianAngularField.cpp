...
py::array_t<double> gasf(py::array_t<double> input) {
    ...
    if (input.ndim() != 2) {
        throw std::runtime_error("Input must be 2-dimensional (n_timestamps, n_timeseries)");
    }
    ...
    for (size_t k = 0; k < n_timeseries; k++) {
        ...
        // GASF: cos(φᵢ + φⱼ) = cos(φᵢ)cos(φⱼ) - sin(φᵢ)sin(φⱼ)
        for (size_t i = 0; i < n_timestamps; i++) {
...
py::array_t<double> gadf(py::array_t<double> input) {
    ...
    for (size_t k = 0; k < n_timeseries; k++) {
        ...
        // GADF: sin(φᵢ - φⱼ) = sin(φᵢ)cos(φⱼ) - cos(φᵢ)sin(φⱼ)
        for (size_t i = 0; i < n_timestamps; i++) {
            for (size_t j = 0; j < n_timestamps; j++) {
                w(i, j, k) = sin_vals[i] * cos_vals[j] - cos_vals[i] * sin_vals[j];
            }
        }
    }
